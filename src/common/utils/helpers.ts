...
import { sendNotification } from '@features/account/account.service';
import { getAdminIds } from '@features/admin/admin.service';
...
export async function sendNotif(accountId: string | string[], code: string, content: Record<string, any> = {}) {
  return await sendNotification(accountId, code, content);
}

export async function sendAdminNotifs(code: string, content: Record<string, any> = {}) {
  return await sendNotification(await getAdminIds(), code, content);
}
...
export function realtime(channelName: string, messageName: string, data: Record<string, any>) {
  ablyRest.channels.get(channelName).publish(messageName, data);
}
